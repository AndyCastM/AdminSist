...
montar_apache(){
    ...
    echo "Apache montado correctamente. Ingrese con http://10.0.0.12:8080"
...
modificar_apache(){
    ...
    mkdir apachev2
    cd apachev2
    echo "<h1>Hola desde Docker, esto lo personalicé yo</h1>" > index.html

    docker run -d --name apache_personalizado -p 8081:80 -v $(pwd)/index.html:/usr/local/apache2/htdocs/index.html httpd
    cd ..
    echo "Apache modificado correctamente. Ingrese con http://10.0.0.12:8081"
